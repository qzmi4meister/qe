# QE 0.7.0 verification

Environment: macOS 26.5.1, arm64, Swift 6.3.2. Checked locally on 16.09.2026. Version 0.7.0 passed 18 core scenarios in debug and release, the full UI suite including keyboard navigation, and the release build. Earlier focused checks, failures, historical measurements, and volume checks are identified separately below.
...
| Debug-only UI checks | Verified in 0.6.3: debug retains the UI runner; release has no `UICheck` symbols or test launch/report/preferences markers. Both 0.7.0 bundle version fields were checked against `VERSION`. |
