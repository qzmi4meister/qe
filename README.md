...
- **Tabs** open with **+** after the last tab and close with the cross inside each tab. **Move Tab to New Window** is available in the tab context menu and the **Window** menu. Moving preserves navigation history, sorting, selection, scroll position, and an active search. Another tab or pane must remain, and the source pane must have no file operation running.
