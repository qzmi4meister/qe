...
- **Search**: enter part of a name and press Return. Click the magnifying glass to choose **This Folder** (direct children only) or **Include Subfolders** (the default). Each pane remembers its scope between launches. Changing scope reruns the current query; clearing the field returns to the directory listing. **Show in Folder** opens a result's parent folder and selects the item.
